...
export default function BalanceSheetAdvancedReport({ type = "monthly-comparison" }) {
  ...
  return <MainLayout><Box sx={{ px: 3, pt: 1, pb: 4 }}>
    ...
    <SectionCard title={<><FaBalanceScale style={{ marginRight: 8, verticalAlign: -2 }} />{title}</>} action={<Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}><Button size="small" variant="contained" startIcon={<FaFileExcel / className="bf-export-button bf-export-excel">} disabled={loading || (isComparison ? !comparisonRows.length : !contractorSummary.length)} onClick={() => exportReport(false)}>Excel</Button><Button size="small" variant="contained" color="error" startIcon={<FaFilePdf / className="bf-export-button bf-export-pdf">} disabled={loading || (isComparison ? !comparisonRows.length : !contractorSummary.length)} onClick={() => exportReport(true)}>PDF</Button></Box>}>
