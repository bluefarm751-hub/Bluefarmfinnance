...
export default function HeadwiseContractorDetailedPdf() {
  ...
  return <MainLayout><Box sx={{ px: 3, pt: 1, pb: 4 }}>
    ...
    <SectionCard title={<><FaFilePdf style={{ marginRight: 8, verticalAlign: -2 }} />Detailed Contractor Bill Report — {farm}</>} action={<Button variant="contained" color="error" startIcon={<FaFilePdf />} disabled={loading || !ordered.length} onClick={buildPdf}>Generate PDF</Button>}>
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} md={5}><TextField fullWidth size="small" select label="Party / Contractor" sx={{ minWidth: 220 }} value={party} onChange={(e) => setParty(e.target.value)}><MenuItem value="__ALL__">All Contractors — Consolidated</MenuItem>{parties.map((p) => <MenuItem key={p.name} value={p.name}>{p.name}</MenuItem>)}</TextField></Grid>
