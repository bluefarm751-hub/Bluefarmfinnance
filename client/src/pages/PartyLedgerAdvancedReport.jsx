  ...
  return <MainLayout><Box sx={{ px: 3, pt: 1, pb: 4 }}>
    ...
    <SectionCard title={<><FaBalanceScale style={{ marginRight: 8, verticalAlign: -2 }} />{titleKind} — {farm}</>}>
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} md={isMonthly ? 5 : 7}><TextField fullWidth size="small" select label="Party / Contractor" sx={{ minWidth: 220 }} value={party} onChange={(e) => setParty(e.target.value)}><MenuItem value="__ALL__">All Contractors — Consolidated</MenuItem>{parties.map((p) => <MenuItem key={p.name} value={p.name}>{p.name}</MenuItem>)}</TextField></Grid>
        ...
        <Grid item xs={12} md={isMonthly ? 3 : 5}><Box sx={{ display: "flex", justifyContent: { xs: "flex-start", md: "flex-end" }, alignItems: "center", height: "100%" }}><Button variant="contained" disabled={!rows.length || loading} startIcon={isExcel ? <FaFileExcel /> : <FaFilePdf />} onClick={exportIt}>{isExcel ? "Download Excel" : "Generate PDF"}</Button></Box></Grid>
      </Grid>
      <Grid container spacing={2} sx={{ mb: 3 }}>
